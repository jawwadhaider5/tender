            ...
            @endcan     
                ...
                <div class="dropdown p-1" style="position: relative;">
                    ...
                    <ul class="dropdown-menu p-2" aria-labelledby="dropdownMenuButton1" style="width:650px; max-width:90vw;">
                        <li>
                            <form method="POST" action="/post-future-client-responds" class="mb-2">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                                ...
                                <div class="row g-2 mb-2">
                                    ...
                                    <div class="col-md-5">
                                        ...
                                        <div style="position: relative;">
                                            <select name="assigned_user_id[]" class="form-control form-control-sm select2 future_client_response_users" multiple style="width: 100% !important;">
                                                {!! $allusers !!} 
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-12 text-end">
                                        <button type="submit" class="btn btn-sm btn-primary" id="submit-future-client-form">Respond</button>
                                    </div>
                                </div>
                            </form>
                        </li>
                    ...
                    foreach ($assigned_user_id as $userId) {
                        if ($userId && $userId != '-' && $userId != '') {
                            try {
                                ...
                                if ($assignedUser) {
                                    $assignedUser->notify(new NotificationsTenderRespond($dt, $user->name, $future_client->subject, $future_client->text));
                                }
                            } catch (\Throwable $notificationException) {
                                // Log notification error but don't break the API
                                Log::warning("Failed to send notification to user ID {$userId} for future client response ID {$future_client->id}: " . $notificationException->getMessage());
            ...
            $citiesData = $futureClients->groupBy(function($futureClient) {
                ...
                $cities = $futureClientsInCode->groupBy(function($futureClient) {
                    ...
                    foreach ($futureClientsByGroup as $groupId => $futureClientsInGroup) {
                        ...
                        $clientsArray = $futureClientsInGroup->map(function($futureClient) {
                            return [
                                'id' => $futureClient->id,
